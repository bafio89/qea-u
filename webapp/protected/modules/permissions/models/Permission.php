 ...
 * @property PermissionsRules $rule
 */
class Permission extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('subject_type, subject_id, rule_id', 'required'),
			array('subject_type', 'length', 'max'=>6),
			array('subject_id, object_id', 'length', 'max'=>20),
			array('rule_id', 'length', 'max'=>10),
			array('object_type', 'length', 'max'=>250),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, subject_type, subject_id, rule_id, object_type, object_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'rule' => array(self::BELONGS_TO, 'PermissionsRules', 'rule_id'),
		);
	}
