...
*/
	...
	function qa_post_create_with_data($type, $parentid, $title, $content, $format='',$date, $categoryid=null, $tags=null, $userid=null, $notify=null, $email=null, $extravalue=null, $name=null)
/*
	Create a new post in the database, and return its postid.
...
*/
	{
		...
		switch ($type) {
			...
			case 'Q_QUEUED':
				$followanswer=isset($parentid) ? qa_post_get_full($parentid, 'A') : null;
				$tagstring=qa_post_tags_to_tagstring($tags);
				$postid=qa_question_create_with_data( $followanswer, $userid, $handle, null, $title, $content, $format, $text, $tagstring,
					$notify,$date, $email, $categoryid, $extravalue, $type=='Q_QUEUED', $name);
				break;

			case 'A':
			case 'A_QUEUED':
				$question=qa_post_get_full($parentid, 'Q');
				$postid=qa_answer_create_with_data($userid, $handle, null, $content, $format,$date, $text, $notify, $email, $question, $type=='A_QUEUED', $name);
				break;
