...
*/
	...
	function qa_db_user_create_by_fb_id($qa_id, $fbid){

		qa_db_query_sub(
			"INSERT INTO ^facebook_id (qa_id, facebook_id) VALUES ($,$)", 
			$qa_id,$fbid
		);
	}

	function qa_db_user_create_by_id($FbId,$email, $password, $handle, $level, $ip)
/*
	Create a new user in the database with $FacebookID, $email, $password, $handle, privilege $level, and $ip address
*/
	{
		...
		qa_db_query_sub(
			...
			'VALUES ($,NOW(), COALESCE(INET_ATON($), 0), $, $, UNHEX($), #, $, NOW(), COALESCE(INET_ATON($), 0))',
			$FbId, $ip, $email, $salt, isset($password) ? qa_db_calc_passcheck($password, $salt) : null, (int)$level, $handle, $ip
		);
...
*/
	...
	function qa_db_user_find_by_facebook_id($fbid){

		
		return qa_db_read_all_values(qa_db_query_sub(
			'SELECT qa_id FROM ^facebook_id WHERE facebook_id=$',
			$fbid
		));
	
	}

	function qa_db_user_find_by_id($id){
