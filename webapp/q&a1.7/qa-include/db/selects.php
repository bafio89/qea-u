...
*/
	...
	require_once QA_INCLUDE_DIR.'util/string-handle.php';
...
// AGGIUNTA DA BAFIO
	function qa_db_categoryslugs_favorite_sql_args($favoriteslugs, &$arguments)
/*
	Return SQL code that represents the constraint of a post being in the category with $categoryslugs, or any of its subcategories
*/
	{	
		...
		if(count($favoriteslugs) == 1){
			...
			foreach($favoriteslugs as $categoryslugs){
				if (!is_array($categoryslugs)) // accept old-style string arguments for one category deep
					$categoryslugs=strlen($categoryslugs) ? array($categoryslugs) : array();
				...
				if (($levels>0) && ($levels<=QA_CATEGORY_DEPTH)) {
					...
					$output = $output . (($levels==QA_CATEGORY_DEPTH) ? 'categoryid' : ('catidpath'.$levels)).'=(SELECT categoryid FROM ^categories WHERE backpath=$ LIMIT 1) OR ';

				}
				
			}
			$output = str_lreplace('OR','' ,$output);
...
//AGGIUNTA DA BAFIO
	function qa_db_favorite_nav_selectspec($slugsfavorite, $isid, $ispostid=false, $full=false)
/*
	Return the selectspec to retrieve ($full or not) info on the categories which "surround" the central category specified
	by $slugsorid, $isid and $ispostid. The "surrounding" categories include all categories (even unrelated) at the
	top level, any ancestors (at any level) of the category, the category's siblings and sub-categories (to one level).
...
*/
	{
		...
		foreach ($slugsfavorite as $slugsorid) {
			...
			if( $i == 0 ){
				...
				$identifiersql= $identifiersql. ' OR ^categories.backpath=$';
				$slugsorid=qa_db_slugs_to_backpath($slugsorid);
			}
			
			for($j = 0 ; $j < 4; $j++){

				array_push($vars, $slugsorid);
			}
...
*/
	...
	//FABIO
	function qa_db_slugs_to_fav_category_id_selectspec($fav_slugs)
/*
	Return the selectspec to retrieve a single category as specified by its $slugs (in order of hierarchy)
*/
	{	
		$i = 0;
		$args = array();
		$source = "^categories WHERE ";


		foreach ($fav_slugs as $slugs){

			if($i == 0 ){

				array_push($args, qa_db_slugs_to_backpath($slugs));
				$source = $source . "backpath=$ ";

			}else{
				array_push($args, qa_db_slugs_to_backpath($slugs));
				$source = $source . "OR backpath=$ ";
			}
			$i++;
		}
