...
*/
	...
	function qa_db_post_create_with_data($type, $parentid, $userid, $cookieid, $ip, $title, $content, $format, $tagstring, $notify, $date, $categoryid=null, $name=null)
/*
	Create a new post in the database and return its ID (based on Facebook ID)
*/
	{
		qa_db_query_sub(
			'INSERT INTO ^posts (categoryid, type, parentid, userid, cookieid, createip, title, content, format, tags, notify, name, created) '.
			'VALUES ( #, $, #, $, #, INET_ATON($), $, $, $, $, $, $, $)',
			$categoryid, $type, $parentid, $userid, $cookieid, $ip, $title, $content, $format, $tagstring, $notify, $name, $date
		);
...
*/
	...
	function qa_post_create_fb_mapping($qa_post_id, $fbid){

		qa_db_query_sub(
					'INSERT INTO ^post_facebook_id (qa_post_id,  qa_post_facebook_id) VALUES ($,$)',
					$qa_post_id, $fbid
				);
	}
	...
	function qa_post_check_by_fb_id($fbid){
		...
		return qa_db_read_all_values(qa_db_query_sub(
			'SELECT qa_post_id FROM ^post_facebook_id WHERE qa_post_facebook_id=$',
			$fbid
		));
	}
